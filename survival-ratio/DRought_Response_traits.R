...
Dat <- read_excel("survival-ratio/Drought_traits_Dat.xlsx")
...
AIC(PC1_model, LDMC_model, SSD_model)

aa<-plot_model(PC1_model, type = "pred",terms=c("PC1","effect"), 
               show.values = TRUE, show.data = TRUE)+ 
  xlab ("PC1")+ylab(NULL)+
  geom_smooth(se=F,linetype=1, size=1)+
  ggtitle("(a)")+
